...
{
    constexpr auto ext_prv_key_bytes = std::array<uint8_t, EXTENDED_KEY_SIZE>{
    ...
    constexpr auto ext_pub_key_bytes = std::array<uint8_t, PUBLIC_KEY_SIZE>{
    ...
    constexpr auto prv_key_bytes = std::array<uint8_t, KEY_SIZE>{
    ...
    constexpr auto pub_key_bytes = std::array<uint8_t, PUBLIC_KEY_SIZE>{
        0xd7, 0x5a, 0x98, 0x01, 0x82, 0xb1, 0x0a, 0xb7, 0xd5, 0x4b, 0xfe,
        0xd3, 0xc9, 0x64, 0x07, 0x3a, 0x0e, 0xe1, 0x72, 0xf3, 0xda, 0xa6,
        0x23, 0x25, 0xaf, 0x02, 0x1a, 0x68, 0xf7, 0x07, 0x51, 0x1a
    };
    const auto pub_key2 = PublicKey(std::span(pub_key_bytes).first<32>());
...
auto testBasicSignature() -> void
{
    ...
    constexpr auto sig_bytes = std::array<uint8_t, SIGNATURE_SIZE>{
...
auto testKeyAdd() -> void
{
    constexpr auto res_pkey = std::array<uint8_t, PUBLIC_KEY_SIZE>{
        0x92, 0xfa, 0xe7, 0x41, 0x5c, 0x3c, 0x8f, 0x6a, 0xe5, 0x0e, 0x19,
        0x83, 0xc3, 0xc5, 0x08, 0xcd, 0x74, 0x0b, 0xde, 0xe5, 0xcd, 0x10,
        0x72, 0xeb, 0xcf, 0xe6, 0x55, 0x9d, 0x84, 0xcd, 0xcc, 0xe1
    };
    constexpr auto res_skey = std::array<uint8_t, KEY_SIZE>{
        0x4e, 0x51, 0xf8, 0xa7, 0x40, 0x4d, 0x4c, 0x0a, 0x34, 0xf5, 0x4a,
        0xb9, 0xd5, 0x1c, 0xbd, 0x68, 0x52, 0x84, 0x95, 0x6f, 0xdf, 0x4c,
        0x83, 0x6f, 0x92, 0x56, 0xed, 0x1a, 0xf8, 0xf7, 0xc7, 0x08
    };
    ...
    constexpr auto ext_pub_key_bytes = std::array<uint8_t, KEY_SIZE>{
        0xaf, 0x65, 0x50, 0xa2, 0x0d, 0x46, 0x89, 0xd5, 0x02, 0xb9, 0x42,
        0xb2, 0x5c, 0xad, 0xf4, 0xe9, 0xa9, 0xa4, 0x4c, 0xc5, 0xf4, 0xca,
        0x0d, 0xdf, 0x05, 0xf1, 0xf5, 0xb6, 0x24, 0x39, 0xd7, 0x69
    };
    ...
    const auto pkey_added = pub_key.pointAdd(pub_key);
    TEST_ASSERT_THROW(pkey_added.bytes() == res_pkey)

    auto skey_added_lower_bytes = ext_prv_key.scalerAddLowerBytes(ext_prv_key);
    TEST_ASSERT_THROW(skey_added_lower_bytes == res_skey)
}
