...
auto testBasic() -> void
{
    auto vrf_key = VRFSecretKey::fromSeed(hex2bytes(test_data[0].seed));
...
auto testAdvanced() -> void
{
    for (size_t i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++)
    {
        ...
        auto vrf_skey = VRFSecretKey::fromSeed(hex2bytes(test_data[i].seed));
        auto vrf_pkey = vrf_skey.publicKey();
        TEST_ASSERT_THROW(vrf_pkey.bytes() == hexToByteArray<32>(test_data[i].pubk));
